...
6. [Δομή Rewards και Reward Shaping](#δομή-rewards-και-reward-shaping)
...
## 6. Δομή Rewards και Reward Shaping

### 6.1 Default Reward (Πρόβλημα)

**Αρχική Συνάρτηση Reward**:
```
reward = -1 για κάθε timestep
```

**Χαρακτηριστικά Default Rewards**:
...
**Το Πρόβλημα με το Sparse Reward**:

Με το default reward (-1 ανά βήμα), ο πράκτορας δεν βλέπει καμία διαφορά μεταξύ καλών και κακών ενεργειών. Με τυχαία εξερεύνηση, το αυτοκίνητο σχεδόν ποτέ δεν φτάνει τον στόχο, οπότε κάθε επεισόδιο τελειώνει με ακριβώς -200 reward. Χωρίς gradient μάθησης, ο PPO δεν μπορεί να βελτιωθεί -- αποτέλεσμα: **0% ποσοστό επιτυχίας**.

### 6.2 Reward Shaping (Λύση)

Για να λύσουμε αυτό το πρόβλημα, χρησιμοποιούμε τον wrapper `ShapedMountainCar` που προσθέτει ενδιάμεσα σήματα ανταμοιβής κατά την εκπαίδευση:
...
class ShapedMountainCar(gym.Wrapper):
    def step(self, action):
        ...
        # 1. Μπόνους νέου ύψους
        if position > self.max_position_seen:
            reward += 10.0 * (position - self.max_position_seen)
            self.max_position_seen = position

        # 2. Μπόνους θέσης (ύψος)
        reward += 0.1 * (position - (-0.5))

        # 3. Μπόνους ταχύτητας (κινητική ενέργεια)
        reward += 5.0 * velocity ** 2

        return obs, reward, terminated, truncated, info
```

**Τα τρία σήματα ανταμοιβής**:

| Σήμα | Σκοπός | Εξήγηση |
|------|--------|---------|
| **Μπόνους νέου ύψους** | Ενθαρρύνει πρόοδο | Ανταμοιβή κάθε φορά που το αυτοκίνητο φτάνει πιο ψηλά από ποτέ στο τρέχον επεισόδιο |
| **Μπόνους θέσης** | Δίνει gradient προς τον στόχο | Μικρή συνεχής ανταμοιβή βάσει ύψους -- "ψηλότερα = καλύτερα" |
| **Μπόνους ταχύτητας** | Διδάσκει τη στρατηγική ταλάντωσης | Ανταμοιβή για κινητική ενέργεια σε οποιαδήποτε κατεύθυνση -- αυτό είναι το κλειδί για να μάθει ο πράκτορας να κουνιέται μπρος-πίσω |

**Σημαντικό**: Η αξιολόγηση γίνεται στο **πραγματικό περιβάλλον χωρίς shaping**, ώστε τα αποτελέσματα να αντικατοπτρίζουν γνήσια απόδοση.
...
### 9.2 Vectorized Environments με Reward Shaping
...
eval_env = gym.make("MountainCar-v0")  # Χωρίς shaping
```

**Εξήγηση**:
- Δημιουργεί 4 παράλληλα περιβάλλοντα με reward shaping για εκπαίδευση
- Το evaluation environment χρησιμοποιεί το αρχικό reward (χωρίς shaping)
- Κάθε περιβάλλον τρέχει ανεξάρτητα, συλλέγει 4x περισσότερη εμπειρία ανά βήμα
...
model = PPO(
    ...
    ent_coef=0.02,            # Bonus exploration
...
- **`ent_coef=0.02`**: Συντελεστής entropy
  - Ενθαρρύνει exploration (ελαφρώς αυξημένος για Mountain Car)
...
Μετά την εκπαίδευση για 500,000 timesteps (~2 λεπτά):

1. **Ο πράκτορας έμαθε τη στρατηγική ταλάντωσης**:
   - Δημιουργεί ορμή κινώντας πρώτα αριστερά (προς τον αριστερό λόφο)
   - Μετά επιταχύνει δεξιά εκμεταλλευόμενος τη βαρύτητα
   - Φτάνει στη θέση στόχου (≥0.5) συνεπώς

2. **Μετρικές Απόδοσης (αξιολόγηση στο πραγματικό περιβάλλον)**:
   - **Ποσοστό επιτυχίας: 100%** (10/10 επεισόδια)
   - Mean reward: **-117.10 ± 2.59**
   - Mean episode length: **~117 βήματα**
   - Καλύτερο επεισόδιο: -112 (112 βήματα)
   - Χειρότερο επεισόδιο: -120 (120 βήματα)

3. **Σύγκριση Πριν/Μετά το Reward Shaping**:

| Μετρική | Χωρίς Shaping | Με Shaping |
|---------|---------------|------------|
| Ποσοστό Επιτυχίας | 0% | **100%** |
| Mean Reward | -200.00 ± 0.00 | **-117.10 ± 2.59** |
| Mean Steps | 200 (πάντα timeout) | **~117** |

4. **Learning Curve**:
   - Αρχικό: Εξερεύνηση με καθοδήγηση reward shaping
   - Μεσαία εκπαίδευση: Μαθαίνει τη στρατηγική ταλάντωσης
   - Τελικό: Συνεπής επιτυχία, `ep_len_mean ≈ 116`
...
- **Phase Space**: Δείχνει ταλαντωτική συμπεριφορά (μπρος-πίσω) -- σπιράλ που μεγαλώνει

**Ανάλυση Actions**:
- **Μαθημένη Πολιτική**:
  - Δεξιά για αρχική μετακίνηση
  - Αριστερά για μεγαλύτερη ταλάντωση (εκμετάλλευση βαρύτητας)
  - Δεξιά με πλήρη ορμή για να φτάσει τον στόχο
  - Ο στρατηγικός συγχρονισμός ενεργειών είναι κρίσιμος

**Ανάλυση Reward Shaping**:
- Τα shaped rewards χρησιμοποιούνται **μόνο κατά την εκπαίδευση**
- Η αξιολόγηση γίνεται στο πραγματικό περιβάλλον (-1 ανά βήμα)
- Το μπόνους ταχύτητας ήταν το κλειδί: δίδαξε στον πράκτορα ότι η κίνηση (σε οποιαδήποτε κατεύθυνση) είναι πολύτιμη
...
Αυτό το project υλοποιεί επιτυχώς το PPO με reward shaping για το περιβάλλον Mountain Car:
...
3. **Reward Shaping**: Ο wrapper `ShapedMountainCar` λύνει το πρόβλημα sparse rewards
4. **Αποτελέσματα**: **100% ποσοστό επιτυχίας**, mean reward -117.10

### Βασικά Συμπεράσματα

1. **Το PPO + Reward Shaping είναι η βέλτιστη λύση** για το Mountain Car:
   - On-policy learning για sample efficiency
   - Σταθερότητα για αξιόπιστη εκπαίδευση
   - Καλή χειριστικότητα discrete actions
   - Το reward shaping παρέχει gradient μάθησης που λείπει από το αρχικό περιβάλλον

2. **Το μεγαλύτερο μάθημα -- τα sparse rewards είναι πρόβλημα**:
   - Χωρίς reward shaping: 0% επιτυχία (ο πράκτορας δεν μαθαίνει τίποτα)
   - Με reward shaping: 100% επιτυχία
   - Η αρχιτεκτονική αλγορίθμου δεν αρκεί αν δεν υπάρχει σήμα μάθησης

3. **Σχεδιαστικές αποφάσεις reward shaping**:
   - Μπόνους ταχύτητας: κρίσιμο -- διδάσκει τη στρατηγική ταλάντωσης
   - Μπόνους θέσης: δίνει gradient προς τον στόχο
   - Μπόνους νέου ύψους: ενθαρρύνει εξερεύνηση ψηλότερων θέσεων
   - Αξιολόγηση χωρίς shaping: εξασφαλίζει γνήσια αποτελέσματα

### Μελλοντικές Βελτιώσεις

1. **Hyperparameter Tuning**: Grid search για βέλτιστες παραμέτρους
2. **Σύγκριση Αλγορίθμων**: Υλοποίηση DQN, A2C για άμεση σύγκριση
3. **Curriculum Learning**: Σταδιακή μείωση reward shaping κατά τη διάρκεια εκπαίδευσης
4. **Transfer Learning**: Δοκιμή στη variant Mountain Car Continuous
